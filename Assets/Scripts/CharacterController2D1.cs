...
public class CharacterController2D1 : MonoBehaviour {
	...
	public ControllerParameters Parameters{ get{return _overrideParameters ?? DefaultParameters;}} //if override parameters is not equal to null return override parameters else return DefaultParameters
	...
	public void LateUpdate(){
		...
		_velocity.y += Parameters.Gravity * Time.deltaTime; // Gravity Component
	...
	private void Move(Vector2 deltaMovement){
		...
		if (StandingOn != null) {
			...
			_activeLocalPlatformPoint = StandingOn.transform.InverseTransformPoint (transform.position); // Convert World space cordinates into local cordinates
			//Debug.DrawLine(Vector3.zero,_activeGlobalPlatformPoint);


			//Handle messages between objects
	...
	private void HandlePlatforms(){
		if (StandingOn != null) {
			var newGlobalPlatformPoint = StandingOn.transform.TransformPoint (_activeLocalPlatformPoint); // Convert local cordinates into World Space cordinates
	...
	private void MoveHorizontally(ref Vector2 deltaMovement){
		...
		for (var i=0; i < TotalHorizontalRays; i++) {
			...
			deltaMovement.x = raycastHit.point.x - rayVector.x; // hitpoint - the origin of the rayVector will give the max distance in the x direction it can traverse without hitting any obstacles
	...
	private void MoveVertically(ref Vector2 deltaMovement){
		...
		rayOrigin.x += deltaMovement.x; // rayOrigin will keep changing as the player moves horizontally
		...
		for (var i=0; i<TotalVerticalRays; i++) {
			...
			if(!isGoingUp && deltaMovement.y > .001f)
	...
	private void HandleVerticalSlope(ref Vector2 deltaMovement){
		...
		var slopeDistance = 5;//SlopeLimitTangent *(_raycastBottomRight.x - center );
